...
import 'package:smartplugin/smartplugin.dart';
...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> androidRootChecker() async {
    try {
      rootedCheck = (await Smartplugin.isRootChecker())!; // return rootcheck status is true or false
  ...
  Future<void> developerMode() async {
    try {
      devMode = (await Smartplugin.isDeveloperMode())!; // return Android developer mode status is true or false
    } on PlatformException {
      devMode = false;
    }
    if (!mounted) return;
